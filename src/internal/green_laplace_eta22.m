function [eta22_plus,audit]=green_laplace_eta22( ...
        eta11_spectrum,qx,qy,peak_depth_qp,J,project_root)
%GREEN_LAPLACE_ETA22 Prescribed-rank pure Green--Laplace eta22 graph.
%
% The operator graph is fixed by the exact Wolfram source. The caller chooses
% the Gauss--Laguerre rank explicitly; no reference field selects or retunes it.
arguments
...
if ~freeze.overall_exact_gate_pass || freeze.oracle_or_mf12_used_in_formula ...
        || freeze.stokes_correction_used || freeze.angular_correction_used
    error('The frozen pure Green--Laplace source failed its authority gate.');
end
...
nu=sqrt(q.*tanh(q)); A=q.*tanh(q); sqrt_A=sqrt(A);
...
for node_index=1:J
    ...
    sinh_over_a=t*ones(size(q)); nonzero=sqrt_A>0;
...
audit=struct('status','prescribed-rank-green-laplace-evaluation', ...
    'quadrature_rank',J,'quadrature_exact_moment_degree',2*J-1, ...
    'fft_ifft_count',10*J+5,'pointwise_product_count',7*J+1, ...
    'learned_coefficients',0,'pair_loops',0,'wolfram_freeze',freeze_path);
end
...
for degree=0:(2*J-1)
    ...
    if relative_error>2e-11
        error('Gauss--Laguerre moment gate failed at J=%d, degree=%d.', ...
            J,degree);
    end
end
end
...
scale=max(active_values); if scale==0,return,end
