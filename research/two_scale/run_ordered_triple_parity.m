function result = run_ordered_triple_parity()
%RUN_ORDERED_TRIPLE_PARITY FFT graph versus direct ordered reconstruction.
...
for ii = 1:numel(modes.kx)
    ix = find(kxv==modes.kx(ii),1);
    iy = find(kyv==modes.ky(ii),1);
...
cases = {'two_scale','shared_scale'};
case_results = struct();
all_pass = true;
for icase = 1:numel(cases)
    cfg = base;
    cfg.mode = cases{icase};
    [~,field_hat,meta] = finite_depth_directional_eta33_two_scale_gl( ...
        eta11_hat,kx,ky,cfg);
    direct = direct_ordered_eta33_two_scale_gl(modes,cfg);
    ...
    for it = 1:size(targets,1)
        tx = targets(it,1);
        ty = targets(it,2);
        expected = sum([direct.records( ...
            [direct.records.kx]==tx & [direct.records.ky]==ty).coefficient]);
        ix = find(kxv==tx,1);
        iy = find(kyv==ty,1);
        actual = field_hat(iy,ix)/(nx*ny);
        discrepancy = abs(actual-expected);
        max_abs = max(max_abs,discrepancy);
        max_rel = max(max_rel,discrepancy/max(abs(expected),1e-30));
    end
    passed = max_abs<2e-18 && max_rel<2e-10;
    ...
    fprintf('%s ordered parity max abs/rel = %.6e / %.6e\n', ...
...
fprintf('artifact = %s\n',artifact);
fprintf('OVERALL_ETA33_TWO_SCALE_GL_ORDERED_PARITY = %s\n',pass_text(all_pass));
if ~all_pass
    error('eta33_two_scale_gl:parity','Ordered-triple parity failed.');
end
end
