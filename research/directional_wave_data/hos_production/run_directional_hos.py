...
r=pathlib.Path('/home/lxy/green-laplace-unidirectional-time-series-runs/hos-directional-fourphase-20260926-v1');mpi=pathlib.Path('/home/lxy/green-laplace-unidirectional-time-series-runs/hos-mpi-check-20260926-v1');base=pathlib.Path('/home/lxy/green-laplace-unidirectional-time-series/artifacts/hos-ocean-v2.1.0')
...
try:
    ...
    shutil.copy2(mpi/'build/sources/HOS-Ocean',r/'bin/HOS-Ocean');assert sha(r/'bin/HOS-Ocean')=='b00f8d20a4d6ba82c472bb239e8577f1e6c1b5a1c6320edb3257831f75c2edde'
    shutil.copy2(base/'deps/usr/bin/time',r/'bin/time');shutil.copy2(source,r/'inputs/initial_fields.mat');shutil.copy2(mpi/'environment.json',r/'environment.json')
    env=os.environ.copy();env.update(json.loads((r/'environment.json').read_text()));env['GFORTRAN_UNBUFFERED_PRECONNECTED']='y';matEnv=os.environ.copy();matEnv.update({k:'1' for k in ['OMP_NUM_THREADS','MKL_NUM_THREADS','OPENBLAS_NUM_THREADS']})
    command="addpath('%s');prepare_directional_hos('%s','%s');"%(r,r,r/'inputs/initial_fields.mat')
    with (r/'prepare.log').open('w') as f:p=subprocess.run([str(r/'bin/time'),'-v','-o',str(r/'prepare-resources.txt'),'/home/lxy/Desktop/matlabr2026a/bin/matlab','-singleCompThread','-batch',command],env=matEnv,cwd=r,stdout=f,stderr=subprocess.STDOUT)
