...
r=pathlib.Path('/home/lxy/green-laplace-unidirectional-time-series-runs/hos-directional-akp002-20260926-v1');mpi=pathlib.Path('/home/lxy/green-laplace-unidirectional-time-series-runs/hos-mpi-check-20260926-v1');base=pathlib.Path('/home/lxy/green-laplace-unidirectional-time-series/artifacts/hos-ocean-v2.1.0')
source=r/'inputs/initial_fields.mat'
np=json.loads((r/'selection.json').read_text())['chosen_ranks']
assert np in [4,8]
...
state={'state':'starting_selected_production','created_utc':utc(),'cases':{},'MPI_ranks_per_case':np,'concurrent_cases':4,'total_compute_processes':4*np,'cpu_sets':{'phi%03d'%p:'%d-%d'%(8+i*np,8+(i+1)*np-1) for i,p in enumerate([0,90,180,270])},'aggregate_RSS_limit_GiB':8}
...
try:
    ...
    assert json.loads((r/'benchmark-validation.json').read_text())['status']=='PASSED_2_SECOND_MPI4_MPI8_AND_PROBE_CHECK'
    ...
    for index,phase in enumerate([0,90,180,270]):
        name='phi%03d'%phase;case=r/'cases'/name;en=env.copy();en['HOS_CPU_BASE']=str(8+np*index);log=(case/'run.log').open('w');logs.append(log)
        args=[str(mpi/'deps/usr/bin/mpirun.openmpi'),'--mca','btl','self,vader,tcp','--bind-to','none','-x','HOS_CPU_BASE','-np',str(np),str(wrapper)]
