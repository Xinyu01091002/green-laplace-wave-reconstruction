function compare_directional_full(run)
out=fullfile(run,'full-gl-comparison-20260926-v1');addpath(fullfile(out,'source','research','directional_wave_data'));addpath(fullfile(out,'source','research','unidirectional_time_series'));
...
report=struct('status','FULL_RECORD_COMPARISON_COMPLETED','snapshot',snapshot,'source_settings',settings,'amplitude_ratios',ratio,'eligible',eligible,'probe_reports',{records},'window_sensitivity',windowAudit,'processing_seconds_by_probe',timers, ...
...
nexttile;plot(t,first(:,1),'Color',[.6 .6 .6]);hold on;plot(t,full.input,'b');xline(full.limits(1),':');xline(full.limits(2),':');grid on;xlabel('Time since startup (s)');ylabel('First harmonic (m)');title(sprintf('Full record: 0--%.1f s',t(end)));legend('Separated','Common input','Location','northwest','Box','off');
for probe=1:5
...
sgtitle('Directional GL time-series reconstruction — full HOS record, common output filtering');exportgraphics(f,fullfile(out,'full_comparison.png'),'Resolution',160);exportgraphics(f,fullfile(out,'full_comparison.pdf'),'ContentType','vector');close(f);disp(allMetrics);disp(windowAudit);
end
