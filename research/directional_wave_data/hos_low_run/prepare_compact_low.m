function report=prepare_compact_low(designFile,mf12Root,out,phaseRandomizationSource,randomSeed)
...
for ip=1:4
    ...
    E(:,:,ip)=eta;P(:,:,ip)=psi;
    name=sprintf('%s_kpd1_akp002_phi%03d',family,phases(ip));folder=fullfile(out,name);mkdir(folder);
    if randomized
...
fid=fopen(name,'w');assert(fid>=0);guard=onCleanup(@()fclose(fid));
fprintf(fid,'Compact directional wavegroup kpd=1 Akp=0.02; independent order-2 initialization\n');
