...
state=json.loads((root/'status.json').read_text());state['resume_hashes']={p.name:hashlib.sha256(p.read_bytes()).hexdigest() for p in [root/'resume_timeseries.py',root/'analyze_hos_timeseries.m']}
...
try:
    for akp in [.02,.12]:
        ...
        if not (root/name/'initial.mat').exists():state['runs'].append(run([matlab,'-singleCompThread','-batch',cmd],root,'prepare-'+name,env));save()
        if akp==.02:
            smoke=root/'probe-smoke';settings=json.loads((root/name/'initialization.json').read_text())
            lines=(smoke/'Results/probes.dat').read_text().splitlines();i=next(i for i,x in enumerate(lines) if x.startswith('VARIABLES'));pv=[float(x) for x in lines[i+1].split()]
