...
本项目已完成初始化，并推进到一阶时间序列的正向重构试验。当前状态见 [STATUS.md](STATUS.md)；下文部分内容保留初始化时的范围说明。
...
2. **已分离 eta1(t)**：已实现逐对/逐三元组时间序列参考执行器，并完成与原空间实现的一致性检查；仍须声明传播方向、水深、记录长度、采样、频谱支持和相位，不将这些有限算例扩展为任意记录的精度认证。
