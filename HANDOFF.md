...
工作目录，详见handoff。随机波OW3D使用原生壁面；HOS周期问题需另行核对边界。
下文保留设计历史，实际启动状态以远程run的 `status.json` 为准。
实际已于2026-09-25 21:37:46 UTC启动，核实16个真实ow3d进程运行、邮件监控
处于waiting。执行源码冻结在 `1d4edd7`；本项目后续HOS修改不会改变该运行副本。
