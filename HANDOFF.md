...
用户指定只保存 eta 和自由面 phi：确认原生 `stride 1` 输出 EP 即可，
不需要 kinematics、Fortran 补丁或重新编译。建议先对现有 kh=1、展宽25、
Akp=.12 做 0.1 s 输出的四相位小批次及 0.2 s 单相位控制，所有原始数据
及后处理留在远程。该网格完整四相位 0.1 s EP 约604.53 GiB，需限制并行
批次数量。完整资源、输出依据、收敛检查和存储预算见
