...
%GREEN_LAPLACE_ETA33_RANKED Rank study of the frozen order-three graph.
% This paper-side evaluator changes only the Gauss--Laguerre node count of
% the frozen eta11-only graph. The inner response generates eta22, flat Phi22,
% and d_t Phi22 from
% damped eta11 leaves. The original order-three kinematic/dynamic forcing is
% then passed through the outer response at the same rank. A symmetric G3
...
if compute_psi
    psi_raw_resolvent_spectrum = psi_spectrum;
    psi_correction = cached_third_order_surface_stokes_correction( ...
        q/3,outer_nodes,outer_weights,lambda3);
    psi_correction(q==0) = 0;
    psi_crossing_correction_spectrum = correction_spectrum.*psi_correction;
    psi_spectrum = psi_spectrum+psi_crossing_correction_spectrum;
end
...
function psi_correction = third_order_surface_stokes_correction( ...
    q,nodes,weights,lambda)
% Exact surface-Psi33 diagonal minus the response at the requested rank.
...
[pair_eta_correction,pair_phi_correction] = pair_stokes_corrections( ...
    q/2,inner_nodes,inner_weights,lambda2);
